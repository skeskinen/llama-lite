...
def embeddings_from_local_server(s):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        data = sock.recv(768*4)
    floats = struct.unpack('f' * 768, data)
    return floats

"""
v1 = np.array(embeddings_from_local_server(''))
v2 = np.array(embeddings_from_local_server('Pillow talk. Main: Leslie, Also: none'))
v3 = np.array(embeddings_from_local_server('Math exam. Main: Ben, Also: Leslie'))

# Compute the cosine similarity between each pair of vectors
similarity_matrix = np.zeros((3,3))
for i, vi in enumerate([v1, v2, v3]):
    for j, vj in enumerate([v1, v2, v3]):
        similarity_matrix[i,j] = np.dot(vi, vj) / (np.linalg.norm(vi) * np.linalg.norm(vj))

print(similarity_matrix)
"""
#embeddings_from_local_server("Hello from Python")
...
def embed_text(text):
    embedding = embeddings_from_local_server(text)
